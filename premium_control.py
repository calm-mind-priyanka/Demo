...
# Convert PREMIUM_USERS expiry strings (if any) to datetime objects on load
...
# Call once on import to fix date parsing
parse_premium_dates()
...
def is_limited_today(user_id):
    ...
    if user_id in DAILY_USAGE and DAILY_USAGE[user_id]['date'] == today:
        return DAILY_USAGE[user_id]['used']  # This should be True or False
    return False
